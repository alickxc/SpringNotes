...
public abstract class AbstractAutoProxyCreator extends ProxyConfig
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) throws BeansException {
		...
		// 此处添加了对targetSource的支持
		// 由于并没完全的了解targetSource的实现及使用此处不瞎说
		if (beanName != null) {
	 ...
	 * @see #shouldSkip
	 */
	// 当前注册的类型是否是不需要增强的如Adice Advisor AopInfrastructureBean
	 ...
	 * @see #buildAdvisors
	 */
	// 创建增强工厂目前Spring使用JDK及CGLIB作为动态增强手段具体实现逻辑在具体类中补充
	protected Object createProxy(
		...
		// 此处着重说明
		// 在向工厂注入接口属性时要先判断是否
		// proxy-target-class被置为true或是否继承了相关接口
		// 如没有注册则没有必要添加接口属性
	...
	protected Advisor[] buildAdvisors(String beanName, Object[] specificInterceptors) {
		...
		// 获得根据名称注册的拦截器
		// 此处逻辑主要为根据Aop处理器中注册的拦截器名称获取Bean并加入到增强集合中
