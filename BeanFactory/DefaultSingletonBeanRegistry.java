...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	// Bean名称与单例Bean实例映射关系用于存储单例对象
	private final Map<String, Object> singletonObjects = new ConcurrentHashMap<String, Object>(64);

	/** Cache of singleton factories: bean name --> ObjectFactory */
	//用于存储Bean与对象工厂关系  
	// **此处用于存储已被实例化但并未包装完成的Bean 
	// **实际用于解决Set方法方式的循环依赖问题
	private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<String, ObjectFactory<?>>(16);

	/** Cache of early singleton objects: bean name --> bean instance */
	// 用于存储当前已被初始化但并未包装完成的Bean的实例对象 理由参上
	...
	// 用于验证当前Bean是否是否需要排除 
	// 其中主要用于当前Bean是否在创建流程执行中
	// 具体参见beforeSingletonCreation，afterSingletonCreation
	...
	// 注册带有销毁流程的Bean
	// 当前Spring支持3种销毁流程
	// 1.destroy-method xml字段
	// 2.Bean继承自DisposableBean接口
	// 3.当前Registry(BeanFactory)中存在DestructionAwareBeanPostProcessor实现
	private final Map<String, Object> disposableBeans = new LinkedHashMap<String, Object>();

	/** Map between containing bean names: bean name --> Set of bean names that the bean contains */
	// 存储Bean的包含关系 
	// 此处逻辑查看子类实现的
	// valueResolver.resolveValueIfNecessary(pv, originalValue); (AbstractAutowireCapableBeanFactory:1419)
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		// 如果在映射中获取单例对象为空且当前Bean正在执行前置流程
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			synchronized (this.singletonObjects) {
				// 向前置映射中申请前置单例对象
				singletonObject = this.earlySingletonObjects.get(beanName);
				// 如果没有命中则申请ObjectFactory生成前置对象
