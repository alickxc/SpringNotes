...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(TransactionDefinition definition) throws TransactionException {
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			// 创建暂停上下文此处因为是初始事务所以TransactionStatus为空
			...
			try {
				...
				// 开启新事务前进行设置如将autocommit置为false等
	 ...
	 */
	// 如果当前已经有事务新建新的TransactionStatus
	private TransactionStatus handleExistingTransaction(
			TransactionDefinition definition, Object transaction, boolean debugEnabled)
			throws TransactionException {

		// 如果事务传播等级为PROPAGATION_NEVER则抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}

		// 如果事务传播等级为PROPAGATION_NOT_SUPPORT则不派生新的事务定义
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
			...
			boolean newSynchronization = (getTransactionSynchronization() == SYNCHRONIZATION_ALWAYS);
			// 此处重要并没有声明新的事务当前事务以非事务形式执行
			return prepareTransactionStatus(
					definition, null, false, newSynchronization, debugEnabled, suspendedResources);
		}

		// 当前事务传播级别为PROPAGATION_REQUIRES_NEW则为事务分配新的TransactionStatus
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			...
			try {
				...
				DefaultTransactionStatus status = newTransactionStatus(
						definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
				// 准备事务
				// 随手一记DataSource的事务管理中为Connection层面
				doBegin(transaction, definition);
				prepareSynchronization(status, definition);
				return status;
			}
		...
		// 事务传播级别为PROPAGATION_NESTED则不另起事务通过还原点的方式进行事务回滚
		...
		// 事务传播级别为PROPAGATION_SUPPORTS或PROPAGATION_REQUIRED
	...
	protected final DefaultTransactionStatus prepareTransactionStatus(
		...
		// 在当前线程下绑定状态定义 
		// 深入TransactionSynchronizationManager中会发现为保证线程间不共享使用ThreadLocal
		prepareSynchronization(status, definition);
		return status;
	}
	...
	protected final SuspendedResourcesHolder suspend(Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				if (transaction != null) {
					// 实际挂起
					// 常用的DataSource挂起逻辑为获取新的Connection放入ConnectionHolder中
					suspendedResources = doSuspend(transaction);
				}
				// 获取当前线程事物上下文信息并绑定至SuspendedResourcesHolder中
			...
			// 如果无活动的同步则直接挂起放入SuspendedResourcesHolder
		...
		try {
			try {
				...
				if (status.hasSavepoint()) {
					...
					// 是否非NESTED传播事务是则回滚
					// 大多数情况下为根事务 
					// 当然有人传播链是REQUIRE->NOT_SUPPORT->NESTED也不违法不是吗
					// 你也吹不爆他拉不长他顶多吐槽他几句难维护呗
					doRollback(status);
				}
