...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 */
	// 此处用于向Spring容器中注册事务增强
	// 我们可以粗略的理解事务管理依赖于Aop实现
	// 其中下方注册的AnnotationTransactionAttributeSource为切点
	// TransactionInterceptor为增强，其中TransActionManager为事务的真正回滚做出支持
	// 在Spring事务管理中数据库层面的事务以连接作为区分
	private static class AopAutoProxyConfigurer {
		...
		public static void configureAutoProxyCreator(Element element, ParserContext parserContext) {
			...
			if (!parserContext.getRegistry().containsBeanDefinition(txAdvisorBeanName)) {
				...
				// 此处注册TransactionAttributeSource用于事务的前置处理
				// 同时在Aop层面其也被用作切点用于在Bean的前置处理流程中
				// 与Bean匹配主要的匹配手段为查找@Transactional注解
				...
				// 中文翻译为事务拦截器用于获得事务在执行过程中出现的各种场景
				// 其中最频繁的为RuntimeException
				...
				// Aop层面的增强事务层面用于将上述组件组装为一个能够使用的事务
